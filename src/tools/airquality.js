// src/tools/airquality.js - get_air_quality_uv tool.

import { getAirQualityUv } from "../lib/openmeteo.js";
import { getCache, cacheKey } from "../lib/cache.js";

export const definition = {
  name: "get_air_quality_uv",
  description: "Get hourly UV index forecast (24/48/72h) for a location from Open-Meteo Air Quality API.",
  inputSchema: {
    type: "object",
    properties: {
...
export async function handler(args, env) {
  ...
  const key = cacheKey({ tool: "uv", lat: args.lat, lon: args.lon, hours });
  const cached = await cache.get(key);
  if (cached) return { content: [{ type: "text", text: JSON.stringify(cached) }] };

  const data = await getAirQualityUv({ lat: args.lat, lon: args.lon, hours });
